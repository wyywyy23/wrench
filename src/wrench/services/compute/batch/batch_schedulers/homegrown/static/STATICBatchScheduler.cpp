...
#include "STATICBatchScheduler.h"
#include "wrench/simulation/Simulation.h"
#include "wrench/logging/TerminalOutput.h"
#include "wrench/services/compute/batch/BatchComputeService.h"

WRENCH_LOG_CATEGORY(wrench_core_static_batch_scheduler, "Log category for STATICBatchScheduler");
...
namespace wrench {
    ...
    */
    std::shared_ptr<BatchJob> STATICBatchScheduler::pickNextJobToSchedule() {
    ...
    std::map<std::string, std::tuple<unsigned long, double>> STATICBatchScheduler::scheduleOnStaticHost(
            long node_idx, unsigned long cores_per_node, double ram_per_node) {
        ...
        if (ram_per_node > Simulation::getHostMemoryCapacity(cs->available_nodes_to_cores.begin()->first)) {
            throw std::runtime_error("STATICBatchScheduler::findNextJobToSchedule(): Asking for too much RAM per host");
        }
        if (cores_per_node > Simulation::getHostNumCores(cs->available_nodes_to_cores.begin()->first)) {
            throw std::runtime_error("STATICBatchScheduler::findNextJobToSchedule(): Asking for too many cores per host");
        }
        ...
        for (map_it = cs->available_nodes_to_cores.begin();
            ...
            if ((*map_it).first == cs->compute_hosts.at(node_idx) and (*map_it).second >= cores_per_node) {
     ...
     */
    std::map<std::string, std::tuple<unsigned long, double>> STATICBatchScheduler::scheduleOnHosts(
        ...
        if (num_nodes > cs->available_nodes_to_cores.size()) {
            throw std::runtime_error("STATICBatchScheduler::findNextJobToSchedule(): Asking for too many hosts");
        }
        if (cores_per_node > Simulation::getHostNumCores(cs->available_nodes_to_cores.begin()->first)) {
            throw std::runtime_error("STATICBatchScheduler::findNextJobToSchedule(): Asking for too many cores per host");
        }
        ...
        if (host_selection_algorithm == "FIRSTFIT") {
            ...
            throw std::invalid_argument(
                    "STATICBatchScheduler::findNextJobToSchedule(): We don't support " + host_selection_algorithm +
    ...
    std::map<std::string, double> STATICBatchScheduler::getStartTimeEstimates(
        ...
        if (cs->getPropertyValueAsString(BatchComputeServiceProperty::HOST_SELECTION_ALGORITHM) != "FIRSTFIT") {
            throw std::runtime_error("STATICBatchScheduler::getStartTimeEstimates(): The static sceduling algorithm can only provide start time estimates "
      ...
      std::cerr << "----------------------------------------\n";
        ...
        for (auto job : this->cs->batch_queue) {
            ...
            // since this is a simple static algorithm with no "jumping ahead" of any kind
            for (auto h : cs->nodes_to_cores_map) {
                std::string hostname = h.first;
                unsigned long num_cores = h.second;
                for (unsigned int i = 0; i < num_cores; i++) {
     ...
     */
    void STATICBatchScheduler::processQueuedJobs() {
        ...
        while (true) {
            ...
            std::map<std::string, std::tuple<unsigned long, double>> resources = {};
	    bool do_static_host = false;

	    // Check if static scheduling
	    std::shared_ptr<StandardJob> standard_job = std::dynamic_pointer_cast<StandardJob>(workflow_job);
	    if (standard_job != nullptr) {
		if (standard_job->getTasks().size() == 1) {
		    if (standard_job->getTasks().at(0)->getStaticHost() != -1) {
		    resources = this->scheduleOnStaticHost(standard_job->getTasks().at(0)->getStaticHost(), cores_per_node_asked_for, ComputeService::ALL_RAM);
		    do_static_host = true;
		    }
		}
	    }
...
//        std::map<std::string, std::tuple<unsigned long, double>> resources;
	    ...
	    if (not do_static_host) {
                resources = this->scheduleOnHosts(num_nodes_asked_for, cores_per_node_asked_for, ComputeService::ALL_RAM);
	    }
...
//                WRENCH_INFO("Can't run job %s right now", workflow_job->getName().c_str());
            ...
            WRENCH_INFO("Starting job %s on %s with %s scheduling", workflow_job->getName().c_str(), resources.begin()->first.c_str(), do_static_host ? std::string("static").c_str() : std::string("fcfs").c_str());
    ...
    void STATICBatchScheduler::processJobSubmission(std::shared_ptr<BatchJob> batch_job) {
        // Do nothing
    }

    void STATICBatchScheduler::processJobFailure(std::shared_ptr<BatchJob> batch_job) {
        // Do nothing
    }

    void STATICBatchScheduler::processJobCompletion(std::shared_ptr<BatchJob> batch_job) {
        // Do nothing
    }

    void STATICBatchScheduler::processJobTermination(std::shared_ptr<BatchJob> batch_job) {
        // Do nothing
    }
